(() => {
  ...
  function decisionSection(proposal) {
    ...
    if (decision === "ACCEPTED") return `<h2>Sua decisão</h2><div class="result"><b>Proposta aceita.</b><p>O aceite desta versão foi registrado.</p>${result.payment?.checkout_url ? `<a class="primary" href="${esc(result.payment.checkout_url)}">Continuar para o pagamento</a>` : `<button id="retry-payment" class="primary">Gerar link de pagamento</button>`}</div>`;

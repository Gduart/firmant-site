(() => {
  ...
  function decisionSection(proposal) {
    ...
    if (decision === "ACCEPTED") {
      ...
      const currentMethod = result.payment?.payment_method || methods[0] || "PIX";
      return `<h2>Sua decisão</h2><div class="result"><b>Proposta aceita.</b><p>Escolha como deseja pagar a entrada.</p><fieldset class="methods"><legend>Forma de pagamento</legend>${methods.map((method) => `<label><input type="radio" name="payment" value="${esc(method)}" ${method === currentMethod ? "checked" : ""}>${paymentName(method)}</label>`).join("")}</fieldset>${methods.includes("CREDIT_CARD") ? `<div id="card-plan" class="card-plan" ${currentMethod === "CREDIT_CARD" ? "" : "hidden"}><label class="field">CPF ou CNPJ do pagador<input id="payer-document" inputmode="numeric" autocomplete="off"></label><label class="field">Parcelamento<select id="installment-count">${Array.from({ length: 12 }, (_, index) => `<option value="${index + 1}">${index + 1}x</option>`).join("")}</select></label><div id="card-quote" class="quote">Calculando taxas atuais do Asaas…</div></div>` : ""}<div id="payment-action">${result.payment?.checkout_url ? `<a class="primary" href="${esc(result.payment.checkout_url)}">Continuar para o pagamento</a>` : `<button id="retry-payment" class="primary">Gerar cobrança segura</button>`}</div></div>`;
    }
    const methods = array(proposal.payment_methods_json);
  ...
  function bindActions() {
    ...
    document.querySelectorAll('input[name="payment"]').forEach((input) => input.addEventListener("change", () => {
      ...
      if (result.acceptance?.decision === "ACCEPTED" && input.value !== result.payment?.payment_method) {
        const action = document.querySelector("#payment-action");
        if (action) { action.innerHTML = '<button id="retry-payment" class="primary">Gerar cobrança segura</button>'; action.querySelector("#retry-payment")?.addEventListener("click", retryPayment); }
      }
    }));
  ...
  async function retryPayment() {
    ...
    try { const paymentMethod = document.querySelector('input[name="payment"]:checked')?.value || result.payment?.payment_method || ""; const installmentCount = Number(document.querySelector("#installment-count")?.value || 1); const payerDocument = document.querySelector("#payer-document")?.value || ""; if (paymentMethod === "CREDIT_CARD" && String(payerDocument).replace(/\D/g, "").length < 11) throw new Error("Informe um CPF ou CNPJ válido."); const response = await fetch(`/api/proposals/${encodeURIComponent(token)}/payment`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentMethod, installmentCount, payerDocument, regenerate: true }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Falha ao gerar pagamento."); location.assign(data.checkoutUrl); } catch (error) { alert(error.message); } finally { busy = false; }
  }

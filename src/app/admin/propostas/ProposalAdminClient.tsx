...
function ContentApprovalCard({ proposalStatus, project }: { proposalStatus: string; project: ProposalDetails["project"] }) {
  return <section className="commercial-admin-card proposal-content-workflow">
    <div className="proposal-content-workflow-header">
      ...
      <span className={`workflow-status workflow-status-${project ? project.status.toLowerCase() : "draft"}`}>{project ? project.status : "AGUARDANDO ACEITE"}</span>
    </div>
    {project ? <>
      <p>O projeto <strong>{project.project_number}</strong> está liberado. Entre no Portal de Conteúdo para enviar imagem, carrossel ou vídeo MP4. Depois do upload, use <strong>Gerar link de revisão</strong> e copie o endereço privado para o cliente.</p>
      <div className="commercial-admin-actions commercial-admin-actions-block">
        <Link className="workflow-primary-action" href={`/admin/conteudos/${project.id}`}>Enviar mídias e gerar link do cliente</Link>
        <Link href="/admin/conteudos">Ver todos os projetos</Link>
      </div>
    </> : <>

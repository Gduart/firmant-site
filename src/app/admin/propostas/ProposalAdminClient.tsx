...
type ProposalRow = {
  ...
  deposit_checkout_expired?: boolean;
...
type Milestone = { id?: string; milestone_type: "FULL" | "DEPOSIT" | "PROGRESS" | "BALANCE" | "ADDITIONAL"; label: string; percentage_basis_points: number | null; amount_cents: number; due_trigger: string; status?: string; payment_method?: string | null; order_id?: string | null; checkout_url?: string | null; paid_at?: string | null; order_status?: string | null; order_created_at?: string | null; checkout_expired?: boolean };
type PaymentHistory = { id: string; milestone_id: string; milestone_label: string; order_id: string; checkout_url: string | null; order_status: string; reason: string; replaced_at: string };
...
export function ProposalAdminClient({ mode, id, briefingId }: { mode: "list" | "editor"; id?: string; briefingId?: string }) {
  ...
  async function createMilestoneCheckout(milestone: Milestone, regenerate = false) {
    ...
    if (regenerate && !window.confirm("Gerar uma nova cobrança? O link anterior continuará apenas no histórico e não deverá mais ser enviado ao cliente.")) return;
    setLoading(true); setError("");
    try {
      const response = await fetch(`/api/admin/proposals/${activeProposalId}/milestones/${milestone.id}/checkout`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ paymentMethod, regenerate }) });
      const data = await response.json();
      ...
      setMessage(data.reused ? "Link de pagamento existente recuperado." : data.replaced ? "Nova cobrança criada. O link anterior foi preservado no histórico." : "Nova cobrança Asaas criada.");
...
function ProposalEditor(props: { details: ProposalDetails; updateProposal: (patch: Partial<ProposalDetails["proposal"]>) => void; updateItem: (index: number, patch: Partial<Item>) => void; setDetails: React.Dispatch<React.SetStateAction<ProposalDetails | null>>; save: () => Promise<boolean>; publish: (sendEmail?: boolean) => Promise<void>; startRevision: () => Promise<void>; createMilestoneCheckout: (milestone: Milestone, regenerate?: boolean) => Promise<void>; setAccessLinkActive: (active: boolean) => Promise<void>; rotateAccessLink: () => Promise<void>; milestoneUrls: Record<string, string>; loading: boolean; setMessage: (value: string) => void }) {
  ...
  </fieldset></section><aside className="commercial-admin-card proposal-editor-sidebar"><h2>Resumo</h2><strong className="proposal-total">{money(totalCents)}</strong><p>{details.items.length} item(ns) · pagamento fixo em 2 etapas</p><section className="proposal-payment-summary"><span>STATUS DA PROPOSTA</span><strong>{proposalStatusLabel(p.status)}</strong><span>STATUS DA ENTRADA (50%)</span><strong className={`payment-status payment-status-${deposit?.checkout_expired ? "danger" : payment.tone}`}>{p.status === "ACCEPTED" ? deposit?.checkout_expired ? "Cobrança expirada" : payment.label : "Aguardando aceite"}</strong><p>{deposit?.paid_at ? `Pagamento registrado em ${new Date(deposit.paid_at).toLocaleDateString("pt-BR")}.` : p.status === "ACCEPTED" ? deposit?.checkout_expired ? "O checkout expirou. Gere uma nova cobrança antes de reenviar ao cliente." : "Produção bloqueada até a confirmação do pagamento da entrada." : "A entrada será cobrada após o aceite do cliente."}</p></section><ProposalAccessLinkControl link={details.accessLink} loading={props.loading} onChange={props.setAccessLinkActive} onRotate={props.rotateAccessLink} setMessage={props.setMessage} />{editable ? <div className="workflow-form-actions"><button type="button" onClick={() => void props.save()} disabled={props.loading}>Salvar rascunho</button><button className="workflow-primary-action" type="button" onClick={() => void props.publish()} disabled={props.loading}>Publicar versão</button><button type="button" onClick={() => void props.publish(true)} disabled={props.loading}>Publicar e enviar por e-mail</button></div> : <><p>Esta versão está protegida contra sobrescrita.</p><div className="commercial-admin-actions commercial-admin-actions-block"><a href={`/api/admin/proposals/${p.id}/pdf`} target="_blank" rel="noreferrer">Abrir PDF</a>{["SENT", "VIEWED", "REJECTED", "EXPIRED"].includes(p.status) && <button type="button" onClick={() => void props.startRevision()}>Iniciar nova versão</button>}</div></>}{p.status === "ACCEPTED" && <><h3>Cobranças por etapa</h3>{details.milestones.map((milestone, index) => { const url = milestone.checkout_url || (milestone.id ? props.milestoneUrls[milestone.id] : ""); const expired = Boolean(milestone.checkout_expired); const state = expired ? { label: "Cobrança expirada", tone: "danger" } : paymentStatusInfo(milestone.status); return <div className="proposal-admin-milestone" key={milestone.id}><span>{index === 0 ? "1ª etapa · 50%" : "2ª etapa · 50%"}</span><strong>{milestone.label}</strong><b>{money(milestone.amount_cents)}</b><span className={`payment-status payment-status-${state.tone}`}>{state.label}</span>{expired ? <button type="button" disabled={props.loading} onClick={() => void props.createMilestoneCheckout(milestone, true)}>Regenerar cobrança</button> : url ? <><a href={url} target="_blank" rel="noreferrer">Abrir cobrança</a>{milestone.payment_method !== "BOLETO" && <button type="button" disabled={props.loading} onClick={() => void props.createMilestoneCheckout(milestone, true)}>Substituir cobrança</button>}</> : <button type="button" disabled={props.loading} onClick={() => void props.createMilestoneCheckout(milestone)}>Gerar cobrança no Asaas</button>}</div>; })}{details.paymentHistory?.length > 0 && <details className="proposal-payment-history"><summary>Histórico de cobranças ({details.paymentHistory.length})</summary>{details.paymentHistory.map((entry) => <div key={entry.id}><strong>{entry.milestone_label}</strong><span>{entry.reason === "EXPIRED" ? "Checkout expirado" : "Checkout substituído"} · {new Date(entry.replaced_at).toLocaleString("pt-BR")}</span>{entry.checkout_url && <a href={entry.checkout_url} target="_blank" rel="noreferrer">Abrir link anterior</a>}</div>)}</details>}</>}<h3>Versões</h3>{details.versions.map((version) => <p key={version.id}>v{version.version_number} · {new Date(version.created_at).toLocaleDateString("pt-BR")}<br /><small>{version.content_hash.slice(0, 16)}…</small></p>)}</aside><ContentApprovalCard proposalStatus={p.status} project={details.project} depositStatus={deposit?.status} /></div>;
}

...
import { rotateProposalAccessLink } from "@/lib/proposals/repository";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return Response.json(await rotateProposalAccessLink({
      proposalId: id,
      actorId: getAdminActor(request),
    }));
  } catch (error) {
    return Response.json(
      { error: error instanceof Error ? error.message : "Falha ao gerar um novo link seguro." },
      { status: 400 },
    );
  }
}

  ...
  async function retryPayment() {
    ...
    try {
      const response = await fetch(`/api/proposals/${token}/payment`, {
        method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin",
        body: JSON.stringify({ installmentCount, payerDocument, regenerate: true }),
      });
      const data = await response.json();
...
function DecisionResult({ decision, checkoutUrl, retryPayment, loading, error, cardFields }: { decision: string; checkoutUrl: string; retryPayment: () => Promise<void>; loading: boolean; error: string; cardFields: React.ReactNode }) {
